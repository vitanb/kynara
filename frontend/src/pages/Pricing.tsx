    ...
    <div className="min-h-screen" style={{ background: "#FFFFFF", color: "#CBD5E1" }}>
      ...
      <nav className="flex items-center justify-between px-8 py-5 max-w-6xl mx-auto">
        <Link to="/" className="flex items-center gap-3">
          ...
          <span className="font-semibold text-ink-50">Kynara</span>
        </Link>
        <div className="flex items-center gap-4">
          <Link to="/login" className="text-sm text-ink-300 hover:text-ink-50 transition-colors">Sign in</Link>
          ...
          return (
            ...
            >
              ...
              <div className="mb-6">
                ...
                <h2 className="text-xl font-bold text-ink-50 mb-1">{plan.name}</h2>
                <p className="text-ink-300 text-sm">{plan.description}</p>
              </div>

              <div className="mb-8">
                <span className="text-4xl font-bold text-ink-50">{plan.price}</span>
                <span className="text-ink-400 text-sm ml-2">{plan.period}</span>
              </div>
      ...
      <div className="border-t max-w-6xl mx-auto px-6 py-16" style={{ borderColor: "rgba(148,163,184,0.08)" }}>
        <h2 className="text-xl font-bold text-ink-50 mb-8">Common questions</h2>
        <div className="grid md:grid-cols-2 gap-8">
          {[
            ...
            <div key={q}>
              <p className="text-ink-50 font-medium mb-2 text-sm">{q}</p>
              <p className="text-ink-300 text-sm leading-relaxed">{a}</p>
            </div>
          ))}
        </div>
      </div>
